...
# Use script name with .txt extension for output
$outputFileName = [System.IO.Path]::GetFileNameWithoutExtension($scriptPath) + ".txt"
$outputFile = Join-Path -Path $scriptDir -ChildPath $outputFileName
...
try {
    ...
    # Parse .sln file and ensure distinct *.csproj project paths
    ...
    $projectPaths = $slnContent |
        ...
        ForEach-Object {
            if ($_ -match '"(.*?)", "(.*?)",') {
                ...
                (Resolve-Path (Join-Path $solutionDir $relativePath | Split-Path)).Path
            }
        } | Group-Object | ForEach-Object { $_.Group[0] }
    ...
    foreach ($projectPath in $projectPaths) {
        ...
        if (Test-Path $projectPath) {
            ...
            $jsonFiles = Get-ChildItem -Path $projectPath -Recurse -Include "appsettings.json", "appsettings.*.json" -File -ErrorAction SilentlyContinue
    ...
    Write-Host "All code and appsettings have been extracted and combined into $outputFile"
} catch {
